...
match wins and `halcyon`/`opus` belong to the parent. **Never hand-edit `backend/knowledge.json`**. It is regenerated by `prebuild`.
...
4. **FastAPI + Gemini**, `backend/main.py`. Optional and currently unhosted. Gemini receives the
   ...
   looked up from that set.

**Tier 3 writes prose; tier 4 still does not.** This was a deliberate relaxation, and it is worth
knowing what replaced the old guarantee. The id-only rule made fabrication impossible and made the
bot answer a question about one detail with a whole topic. What holds tier 3 now, in order of how
much it actually protects:

- **Guards run first**, so a sensitive, abusive, personal or off-topic question never leaves the
  Worker, whatever the conversation looks like. `modelEligible` allows only `'From the portfolio'`
  and the no-match fallback.
- **The model is handed nothing but a shortlist of approved answers** and told to use nothing else.
  This, not the citation list, is the real constraint: it cannot quote what it was not given.
- **A link it invents is rejected**, which is the one fabrication that costs a visitor something.
- **A reply that declines is rejected too** (`NON_ANSWER`). The model reading the not-documented
  answer back out of its own shortlist, or narrating its scaffolding ("The sources do not
  provide…"), both shipped to production labelled `AI · grounded in portfolio`, which tells a
  visitor the model weighed the question and declined. Returning null hands it to the curated
  fallback, which says the same thing in the site's voice and under a true source.
- **Any failure returns the built-in answer**, verbatim, as before. The offline tier in `Chat.tsx`
  is unchanged and always serves curated text.
...
tier is serving anybody: on low traffic it may never have run at all. Two causes: real visitors ask one question and
leave, so `enrichable && history.length > 0` is never true for them; and when `unmatched` does
reach the model it frequently misses the 8.5s window. Raising the client timeout or streaming the
reply are the real options, and both are UX decisions rather than bug fixes.

**`npm run check:model` is the only local exercise of this tier.** `npm run test:chat` replays its
cases against a server whose answers all take the regex path, so tier 3 is untested by
construction; every defect above had to be found in production. The script calls `composeAnswer`
directly, reports the answer rate and p95, and exits non-zero if any reply lands after 8.5s.

The `used` list the model returns is **not** a safeguard, and `lib/chat/nim.ts` says so: the model
only ever sees the shortlist, so a citation it declares adds nothing the context did not. It is
kept because it tells the *next* turn what this answer was about.

`Answer.unmatched` exists to keep the eligibility rule honest: `guard.unknown` (salary, CTC) and
the final no-match fallback both answer with the source `'Not documented'`, so the source string
cannot tell a refusal from a miss, and only the miss may reach a model.

**There is no RAG here and there should not be.** The whole corpus is ~104 answers, ~16k tokens,
and it fits in a prompt. The shortlist in `nim.ts` exists to keep the prompt small and the
grounding tight, and it is IDF-weighted token overlap, about twenty lines. Plain overlap was tried
and ranked the Best Project award above the Lumen entry for an Lumen question, on the word
"project" alone. `/chat` also enforces an optional bearer token (`CHAT_BACKEND_TOKEN`), a
